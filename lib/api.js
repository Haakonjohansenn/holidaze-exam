  ...
  try {
    ...
    if (!response.ok) {
      if (response.status === 409) {
        throw new Error("This date range is already booked. Please select different dates.");
      }
...
export const fetchBookingsForVenue = async (venueId) => {
  try {
    ...
    const response = await fetch(`https://v2.api.noroff.dev/holidaze/venues/${venueId}?_bookings=true&_customer=true&_venue=true`, {
      headers: {
        'Authorization': `Bearer ${accessToken}`,
        'X-Noroff-API-Key': apiKey,
      },
    ...
    if (!response.ok) {
      throw new Error("Failed to fetch bookings for venue");
    }
    const data = await response.json();
